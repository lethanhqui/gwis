...
class WPAS_File_Upload {
	...
	public function upload_field() {
		...
		$attachments_args = apply_filters( 'wpas_ticket_attachments_field_args', array(
			...
			'args' => array(
				...
				'field_type' => 'upload',
				'multiple'   => true,
				'label'      => __( 'Đính Kèm', 'awesome-support' ),
				'desc'       => sprintf( __( ' Bạn có thể tải lên tối đa %d files (tối đa %d MB mỗi file) các loại sau đây: %s', 'awesome-support' ), (int) wpas_get_option( 'attachments_max' ), (int) wpas_get_option( 'filesize_max' ), apply_filters( 'wpas_attachments_filetypes_display', $filetypes ) ),
			),
		) );
	...
	public function show_attachments( $post_id ) {
		...
		if ( ! empty( $attachments ) ): ?>
			...
			<div class="wpas-reply-attachements">
				<strong><?php _e( 'Đính Kèm:', 'awesome-support' ); ?></strong>
				<ul>
					<?php
	...
	public function limit_upload( $file ) {
		...
		if ( ! in_array( $ext, $filetypes ) ) {
			$file['error'] = sprintf( __( 'Bạn không được phép tải lên các file kiểu này (%s)', 'awesome-support' ), $ext );
		}

		if ( $file['size'] <= 0 ) {
			$file['error'] = __( 'Bạn không thể tải file đính kèm trống. Bạn đính kèm trọng lượng 0 byte', 'awesome-support' );
		}

		if ( $file['size'] > $max_size_bytes ) {
			$file['error'] = sprintf( __( 'Đính kèm của bạn là quá lớn. Bạn được phép đính kèm file lên đến %s', 'awesome-support' ), "$max_size Mo" );
		}
